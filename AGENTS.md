...
python -m backend.evals.eval_module2   # researcher / grounding / dispatcher / collector tests
...
python -m backend.evals.eval_quality   # LLM-as-Judge quality evaluation
python -m backend.evals.eval_pipeline  # end-to-end live pipeline tests (uses XML task files)
```
...
ALLOWED_ORIGINS=          # Optional: comma-separated extra CORS origins (Railway app domains auto-allowed)
```
...
| POST | `/api/research/cleanup` | Manual purge of oldest records (body: `{"keep": N}`) |
...
### Discovery ([backend/services/discovery_service.py](backend/services/discovery_service.py))
...
### PDF ([backend/services/pdf_service.py](backend/services/pdf_service.py))
Handles PDF generation for report downloads (`GET /api/research/{id}/download` with `format=pdf`).
